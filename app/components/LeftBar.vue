...
<template>
 <div class="  fixed w-full md:w-20 md:h-full md:top-0 md:left-0 bottom-0 z-10 bg-background-primary text-white">
  <ul class="flex md:flex-col flex-row items-center md:mt-10 gap-6">
   <router-link v-for="item in menuItems" :key="item.path" :to="item.path" custom v-slot="{ navigate, isActive }">
    <li @click="navigate" class="h-20 w-full flex justify-center items-center flex-col
     ...
     <div v-if="isActive" class="absolute bottom-0 left-1/2 -translate-x-1/2 flex flex-col items-center">
      <div class="w-[25px] h-[23px] bg-primary blur-xl absolute bottom-[17px]" />
